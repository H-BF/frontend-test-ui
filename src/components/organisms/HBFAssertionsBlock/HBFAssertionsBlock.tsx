...
import { filterData, filterForSelected } from './utils'
import { Styled } from './styled'

type THBFAssertionsBlockProps = {
  id: string
}

export const HBFAssertionsBlock: FC<THBFAssertionsBlockProps> = ({ id }) => {
  const [error, setError] = useState<TRequestError | undefined>()
  const [isLoading, setIsLoading] = useState<boolean>(false)
  const [data, setData] = useState<TAssertion[]>([])
  const [filteredData, setFilteredData] = useState<TAssertion[]>([])
  ...
  useEffect(() => {
    ...
    setError(undefined)
    setData([])
    getAllAssertionsData(id)
      .then(({ data }) => {
        setIsLoading(false)
        setData(data.assertions)
      })
